# tf-atom-security-group-rule-aws

[![CI](https://github.com/PlatformStackPulse/tf-atom-security-group-rule-aws/actions/workflows/ci.yml/badge.svg)](https://github.com/PlatformStackPulse/tf-atom-security-group-rule-aws/actions/workflows/ci.yml)
![Terraform](https://img.shields.io/badge/terraform-%3E%3D1.6.0-blueviolet)

## Purpose

Terraform atom: AWS Security Group Rule - adds an ingress or egress rule to a security group.
...
|------|-------------|------|---------|:--------:|
| <a name="input_from_port"></a> [from\_port](#input\_from\_port) | Start port (use -1 for all) | `number` | n/a | yes |
| <a name="input_rule_type"></a> [rule\_type](#input\_rule\_type) | Type of rule (ingress or egress) | `string` | n/a | yes |
| <a name="input_security_group_id"></a> [security\_group\_id](#input\_security\_group\_id) | ID of the security group | `string` | n/a | yes |
| <a name="input_to_port"></a> [to\_port](#input\_to\_port) | End port (use -1 for all) | `number` | n/a | yes |
| <a name="input_attributes"></a> [attributes](#input\_attributes) | ID element. Additional attributes (e.g. `workers` or `cluster`) to add to `id`,<br/>in the order they appear in the list. New attributes are appended to the<br/>end of the list. The elements of the list are joined by the `delimiter`<br/>and treated as a single ID element. | `list(string)` | `[]` | no |
| <a name="input_cidr_ipv4"></a> [cidr\_ipv4](#input\_cidr\_ipv4) | CIDR block to allow (mutually exclusive with referenced\_security\_group\_id) | `string` | `null` | no |
...
| <a name="input_ip_protocol"></a> [ip\_protocol](#input\_ip\_protocol) | Protocol (tcp, udp, icmp, -1 for all) | `string` | `"tcp"` | no |
...
| <a name="input_referenced_security_group_id"></a> [referenced\_security\_group\_id](#input\_referenced\_security\_group\_id) | Security group ID to allow (mutually exclusive with cidr\_ipv4) | `string` | `null` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_egress_rule_id"></a> [egress\_rule\_id](#output\_egress\_rule\_id) | ID of the egress rule |
